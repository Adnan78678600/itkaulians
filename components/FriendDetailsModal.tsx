...
const FriendDetailsModal: React.FC<FriendDetailsModalProps> = ({ friend, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (!modalRef.current || !cardRef.current) return;

    // Set initial states immediately before animation
    gsap.set(modalRef.current, { opacity: 0 });
    gsap.set(cardRef.current, { y: 40, opacity: 0, scale: 0.96 });

    // Set animate-item elements to visible immediately (no animation issues)
    const animateItems = cardRef.current.querySelectorAll(".animate-item");
    gsap.set(animateItems, { y: 0, opacity: 1 });
    ...
    return () => {
      ...
      tl.kill();
      // Ensure content is visible if animation was killed mid-way
      gsap.set(animateItems, { y: 0, opacity: 1 });
